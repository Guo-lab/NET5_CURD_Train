...
namespace ProjectBase.BusinessDelegate
{
                                                                          ...
                                                                          where TId : struct
    {
        ...
        private Expression ChainedPropExpr(Expression expr, string propName, Type containerType, Type dtoPropType)
        {
            ...
            Expression rtn= ChainedPropExpr(firstPart, sencondPart, entityProp.PropertyType, dtoPropType);
            if (rtn==null || (dtoPropType.IsValueType && !Util.IsNullableType(dtoPropType)))
                return rtn;

            return Expression.Condition(Expression.Equal(firstPart, Expression.Constant(null)),
                 Expression.Constant(null, dtoPropType), Expression.Convert(rtn, dtoPropType));
        }
