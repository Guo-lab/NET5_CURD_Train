...
namespace ProjectBase.AutoMapper
{
    ...
    public class AutoMapperProfile : Profile
    {
        ...
        protected void CreateMapForSelfMapper()
        {
            ...
            {
                var types = assembly.GetTypes()
                                .Where(t => t.GetInterfaces().Any(f => f.FullName.StartsWith(interfaceName)));
                foreach (var t in types)
                {
                    var srcType = t.GetInterfaces().Where(f => f.FullName.StartsWith(interfaceName)).Single().GetGenericArguments()[0];
                    CreateMap(srcType,t);
                }
            }
        }
