...
export class ProjectBaseService {
    ...
    private setResultMarker(rcJsonResult: RcResult) {
        if (!rcJsonResult) {
    ...
    async Validate(formGroup: VmForm, valGroups?: string | string[], partialValid?: string | string[]): Promise<string> {
        const err = await this.autoValidator.validate(formGroup as VmForm, valGroups, partialValid);
        if (!err) {
            return (formGroup as PbAbstractControl).pbResolvedErrors && (formGroup as PbAbstractControl).pbResolvedErrors.classValidate ?
    ...
    */
    CreateFormArray(containingFormGroup: VmForm, arrayPropNameOrVmInputTypeNameOfEle: string, arrayValue: any[] | number, classValidator?: ClassValidator): FormArray {
        ...
        if (formArray && formArray.controls && formArray.controls.length > 0) {// containingFormGroup的相应子节点已经创建为FormArray，进行初始赋值
    ...
    private addFormToken(formToken:string) {
        ...
        if(channel==='header'&&formToken){
            ...
            const obj:any={};
            obj[ClientServerConst.NAME_FORMTOKEN]=formToken;
    ...
    private async addLoginToken(loginToken: string) {
        ...
        if(channel==='header'&&loginToken){
            ...
            const obj:any={};
            obj[ClientServerConst.NAME_LOGINTOKEN]=loginToken;
        ...
        const afterVal = () => {
            ...
            if (!formValueContainingProp) {
                if (ajaxConfig.postJson) {
                    ...
                    formData = (formData as string).replaceAll("Input.", "");
                }
            }
    ...
    private async doSubmit(url: string, method: string, ajaxBodyData: any, hostComponentInstance: VmComponentBase | any, form?: VmForm, ajaxConfig?: AjaxSubmitConfig,
        ...
        */
        ...
        const byRequestLoginToken=await this.addLoginToken(ajaxConfig.loginToken);
        ...
        let promise = request.then((response: any[]) => {
            if (response[1]) {
                if (response[1].statusCode == 200) {
                    ...
                    throw { status: response[1].statusCode };
                }
            } else {
                throw new Error('http响应格式异常:' + response);
            }
    ...
    private serializeAngularForm(formGroup: VmForm, formValueContainingProp: string, postJson = true): string {
        if (postJson) {
    ...
    private buildVmMetaFb(vmInputTypeName: string, validateOn?: ValidateOnOpt, innerValidators?: {
        ...
        if (!meta) {
            throw new Error(`VmMeta data not found for vmInputTypeName "${vmInputTypeName}"`);
        }
    ...
    private recursiveBuildVmMetaFb(meta: VmMeta, nodeName: string, parentMeta?: VmMeta, justForLeafEle = false, validateOn?: ValidateOnOpt, innerValidators?: {
        ...
        if (this.modelTypeIsArray(meta.modelType) && !justForLeafEle) {// array prop
            ...
            metaFbPointer = {
                pbRuleConfig: controlsConfig === {} ? null : controlsConfig,
                controls: controls
            };
        } else {
            if (validators) {
    ...
    public getValidators(meta: VmMeta, justForLeafEle = false, valGroups?: string | string[], innerValidators?: {
        [name: string]: ValidatorFn;
    }) {
        ...
        if (rules) {
            for (const rulekey of Object.keys(rules)) {
                ...
                if (validator) {
                    ...
                    if (!f) throw new NgxArchError('未注册类内部验证器：' + rule.type);
    ...
    private shouldVal(valRule: ValidationRule, valGroups?: string | string[]) {
        ...
        if (!valGroups) {
            return !valRule.groups || ruleGrps.includes(ClientServerConst.VmMeta_DefaultRuleGroup);
        }
        if (typeof valGroups === 'string') valGroups = [valGroups];
        let should = false;
        valGroups.forEach((grp, i) => {
            if (ruleGrps.includes(grp)) {
                should = true;
                return false;
            }
        });
