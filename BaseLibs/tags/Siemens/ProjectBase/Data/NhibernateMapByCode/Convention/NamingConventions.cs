...
namespace ProjectBase.Data.NHibernateMapByCode.Convention
{
    public static class NamingConventions
    {
        ...
        /// 默认映射集合时是否设置级联操作。
        /// </summary>
        public static bool DefaultCascade { get; set; } = false;

        /// <summary>
        /// Id自动生成算法中，每插入几行同步一次hibernate_sequences表
        /// </summary>
        public static int SeqIdGeneratorIncrementSize { get; set; } = 100;
        ...
        public static void MapProperty(IModelInspector modelInspector, PropertyPath member, IPropertyMapper map)
        {
            ...
            DateTimeToTimeStampConvention(modelInspector, member, map);
        }
        ...
        {
            var property = member.LocalMember as PropertyInfo;
            if (property!=null && property.Name == "RefText")
            {
                ...
                {
                    var prop = refTextProp.First();
                    var formula = Attribute.GetCustomAttribute(prop, typeof(RefTextAttribute)) as RefTextAttribute;
                    map.Formula(string.IsNullOrEmpty(formula.Formula) ? prop.Name : formula.Formula);
                }
                else
                {
                    string s = "";
        ...
        public static void DateTimeToTimeStampConvention(IModelInspector modelInspector, PropertyPath member, IPropertyMapper map)
        {
            var property = member.LocalMember as PropertyInfo;
            if (property != null && (property.PropertyType ==typeof(DateTime) || property.PropertyType == typeof(DateTime?)))
            {
                map.Type<NHibernate.Type.TimestampType>();
            }
        }
        ...
        public static void MapBag(IModelInspector modelInspector, PropertyPath member, IBagPropertiesMapper map)
        {
            ...
            if (inv == null)
            {
                ...
                if (DefaultCascade)
                {
                    map.Cascade(Cascade.All | Cascade.DeleteOrphans); //默认不级联，级联使得性能差
                }
                map.BatchSize(20);
                map.Inverse(true);
        ...
        {
            if (typeof(DomainObjectWithAssignedIntId).IsAssignableFrom(type))
            {
                map.Id(k =>
                {
                    k.Generator(Generators.Assigned);
                    k.UnsavedValue(0);
                });
            }else if (typeof(DomainObjectWithAssignedGuidId).IsAssignableFrom(type))
            {
                map.Id(k =>
                {
                    k.Generator(CustomGenerators.GuidGenerator); //CommonBD.Save的时候会判断是否已经赋值
                    k.UnsavedValue(Guid.Empty);
                });
            }
            else if (typeof(BaseDomainObjectWithSeqId).IsAssignableFrom(type))
            {
                map.Id(k =>
                {
                    k.Generator(Generators.EnhancedTable, (gm)=> {
                        gm.Params(new
                        {
                            prefer_entity_table_as_segment_value = true,
                            optimizer = "pooled-lo",
                            increment_size= SeqIdGeneratorIncrementSize
                        });
                    }); 
                    k.UnsavedValue(0);
                });
        ...
        public static void VersionConvention(Type type, IClassAttributesMapper map)
        {
            if (typeof(ProjectBase.Domain.NhibernateMapByCode.IVersion).IsAssignableFrom(type))
            {
                map.Version(type.GetProperty("RowVersion"), m =>
                {
