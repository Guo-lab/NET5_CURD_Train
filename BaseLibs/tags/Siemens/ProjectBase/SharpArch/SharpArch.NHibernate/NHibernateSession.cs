namespace SharpArch.NHibernate
{
    ...
    public static class NHibernateSession
    {
        ...
        /// <summary>
        ///    在hibernate配置文件中指定的connection.isolation=readuncommitted本身只对事务起效，为支持对无事务的语句起效，框架在OpenSession时增加执行一条设置语句set transaction isolation level。
        ///    同时增加NHibernateSession.ApplyDefaultIsolation设置是否开启此项支持。
        /// </summary>
        public static bool ApplyDefaultIsolationToConnection { get; set; } = false;
        /// <summary>
        ...
        private static Dictionary<string, IsolationLevel> defaultIsolationLevels =
            new Dictionary<string, IsolationLevel>();
        ...
        ************/
            ...
            string factoryKey, ISessionFactory sessionFactory, Configuration cfg, string validatorCfgFile)
        {
            ...
            var prop = cfg.Properties["connection.isolation"];
            if (prop.Equals("1")|| prop.Equals("readuncommitted",StringComparison.OrdinalIgnoreCase))
            {
                defaultIsolationLevels.Add(factoryKey, IsolationLevel.ReadUncommitted);
            }
        ...
        public static void CloseAllSessionsAndClearStorage()
        {
            CloseAllSessions();
            foreach (var key in Storage.GetAllKeys())
            {
                Storage.RemoveSessionForKey(key);
            }
        }
        ...
        {
            return CurrentFor(factoryKey, ApplyDefaultIsolationToConnection);
        }
        public static ISession CurrentFor(string factoryKey, bool setDefaultIsolation)
        {
            ...
            if (session == null)
            {
                ...
                if (setDefaultIsolation && defaultIsolationLevels.ContainsKey(factoryKey)&&defaultIsolationLevels[factoryKey]==IsolationLevel.ReadUncommitted)
                {
