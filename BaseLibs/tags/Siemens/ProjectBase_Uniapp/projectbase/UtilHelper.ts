...
export class UtilHelper {
     ...
     */
    MetaFilter(enclosingObj: any, nameofPropToFilter: string, metaForIgnore: VmMeta, checkFormIgnore = false, checkSubmitIgnore = true) {
    ...
    ToDoWhen(condition: () => boolean, todo: () => void,timeoutCallback?:()=>void, timeoutMillisec?: number) {
        ...
        if (timeoutMillisec < 0 || timeoutMillisec === 0) {//不能用timeoutMillisec<=0,因为null<=为true,而null<0 || null==0为false
            if (timeoutCallback) {
                timeoutCallback();
            } else {
