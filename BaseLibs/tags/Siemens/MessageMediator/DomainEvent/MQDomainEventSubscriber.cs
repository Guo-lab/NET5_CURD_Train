...
namespace MessageMediator.DomainEvent
{
    public class MQDomainEventSubscriber: IDomainEventOutProcSubscriber
    {
        ...
        //近期处理过的消息，用于防止重复消息处理
        private ConcurrentDictionary<Guid, DateTime> recentMsgs = new ConcurrentDictionary<Guid, DateTime>();

        private IConnection? connection;
        private string subscriberName;
        private string domainEventNS;
        private int clearInterval = 60 * 60 * 1000;//每小时;

        private Timer clearAckTimerReferenceHolder;

        public void Setup(string subscriberName, string publisherNames, string domainEventNS) 
        {
            ...
            {
                AlarmReporter.Error("DomainEvent订阅异常：", e);
            }
            clearAckTimerReferenceHolder = new Timer((state) => ClearRecentMsgInProcCache(), null, 0, clearInterval);
        }
        ...
        private bool IsDuplicate(DomainEventWrapper wrapper)
        {
            return recentMsgs.ContainsKey(wrapper.EventId);
        }

        private void AddRecentMsg(DomainEventWrapper wrapper)
        {
