...
namespace ImportLibrary
{
    ...
    public class BaseImportResult<T> where T : BaseImportItem
    {
        ...
        public FeedbackResult<T> GetFeedBackResult()
        {
            if (this.ImportResult == ImportResult.InvalidFile)
            {
                if (string.IsNullOrEmpty(this.ErrorMessage))
                {
                    return new FeedbackResult<T> { ErrorMessage = "请检查文件格式是否正确" };
                }
                else
                {
                    return new FeedbackResult<T> { ErrorMessage = this.ErrorMessage };
                }
            }
            //else if (this.ImportResult == ImportResult.UnexpectedResult)
            //{
