...
namespace KendoUIHelper
{
    public static class HtmlExtensions
    {
            ...
            where TControlBuilder : ControlBuilderBase<TControl, TControlBuilder>
        {
            ...
            SetEnableByVMAnnotation(htmlHelper, expression, builder);
        }
        //根据VM标记设置锁定
        
        private static void SetEnableByVMAnnotation<TControl, TControlBuilder, TModel, TProperty>(IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, ControlBuilderBase<TControl, TControlBuilder> builder)
            where TControl : ControlBase
            where TControlBuilder : ControlBuilderBase<TControl, TControlBuilder>
        {
            var expresionProvider = HtmlHelperUtil.GetExpressionProvider(htmlHelper);
            ModelMetadata meta = HtmlHelperUtil.MetaFromExpression(expresionProvider, htmlHelper, expression);
            var enableAttr = Attribute.GetCustomAttribute(meta.ContainerType.GetProperty(meta.PropertyName), typeof(BaseFieldControlAttribute), true) as BaseFieldControlAttribute;
            if (enableAttr != null) {
                if (enableAttr.ReadOnly.HasValue)
                {
                    if (!enableAttr.ReadOnly.Value) { builder.ReadOnly("true"); }               
                }
                else {
                    if (string.IsNullOrEmpty(enableAttr.ListName) == false)
                    {
                        if (string.IsNullOrEmpty(enableAttr.FieldName) == false)
                        {
                            builder.ReadOnly(htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.ListName + "['" + enableAttr.FieldName + "']");
                        }
                        else
                        {
                            builder.ReadOnly(htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.ListName + "['" + meta.PropertyName + "']");
                        }           
                    }
                    else {
                        builder.ReadOnly(htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.FieldName);
                    }
                }
