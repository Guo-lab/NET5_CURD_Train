        ...
        var ExecuteErrorResult = function (rcJsonResult) {
            if (rcJsonResult.IsMsg) {
                ...
                $translate(data).then(function (msg) {
                    var TranslateParam = function (params, index) {
                        var d = $q.defer();
                        $translate(params[index]).then(function (result) {
                            msg = msg.replace("{" + index + "}", result);
                            if (index == params.length - 1) {
                                d.resolve(msg);
                            } else {
                                TranslateParam(params, index + 1);
                            }
                        }, function (original) {
                            msg = msg.replace("{" + index + "}", original);
                            if (index == params.length - 1) {
                                d.resolve(msg);
                            } else {
                                TranslateParam(params, index + 1);
                    ...
                    if (params.length == 0) {
                        ...
                        TranslateParam(params, 0).then(function (msg) {
                            pbui.ShowCommand(rcJsonResult.command, msg);
                        })
                    }
