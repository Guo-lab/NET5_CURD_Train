...
namespace SocketBase
{
    ...
    public class SocketSessionStorage : ISessionStorage
    {
        ...
        //<httpcontext及其启动的异步线程，共有线程池，因此 static 的线程变量需注意自行为每个请求和异步任务做一次初始化。见下面UsageOutofHttpContext_Begin和Application_BeginRequest
        private static ThreadLocal<bool> sessionIsUsedOutofRequestContext;
        ...
        public virtual RequestListenerRegistra Registra
        {
            ...
            {
                registra = value;
               // registra.AddBeginHandler(Application_BeginRequest, Application_BeginRequest_Order);TODO：为避免意外，此行暂不生效，等必要时再
        ...
        {
            //TODO：为避免意外，以下暂不生效，等必要时再
            // sessionIsUsedOutofRequestContext = new ThreadLocal<bool>(() => true);
            // storageForTaskThread = null;
        }
        public static void Application_BeginRequest(RequestContext context)
        {
            //TODO：为避免意外，以下暂不生效，等必要时再
            //sessionIsUsedOutofRequestContext = new ThreadLocal<bool>(() => false);
            //storageForTaskThread = null;
        }
