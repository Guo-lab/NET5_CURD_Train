(function (String, angular) {
    ...
    var pbuiFn = ['$uibModal', '$q', '$translate', '$uibPosition', '$rootScope', '$window', '$log',
        function ($uibModal, $q, $translate, $uibPosition, $rootScope, $window, $log) {
            var PutProcessing = function (elementIdOrElement) {
                if (elementIdOrElement == false) {
                    if ($rootScope && $rootScope.pbvar) {
                        $rootScope.pbvar.ShowProcessing = false;
                    }
                    return;
                }
                if (!elementIdOrElement) elementIdOrElement = angular.element('body');
                var element;
                if (typeof elementIdOrElement == 'string') {
                    var elementId = elementIdOrElement;
                ...
                if (!element) {
                    $rootScope.pbvar.ShowProcessing = false;
                    $log.error('no element for ' + elementIdOrElement);
                    return;
                }
                var padding = 5;
                var pos = $uibPosition.position(element);
                $rootScope.pbvar.ProcessingRect.width = (pos.width + padding * 2) + 'px';
                $rootScope.pbvar.ProcessingRect.height = pos.height > 30 ? (pos.height + padding * 2) + 'px' : 50 + 'px';
                var top = 0, left = 0;
                ...
                $rootScope.pbvar.ProcessingRect.top = (top - padding) + 'px';
                $rootScope.pbvar.ProcessingRect.left = (left - padding) + 'px';
                $rootScope.pbvar.ShowProcessing = true;
            };
            var AlertModal = function (msg, msgParams, size, animationsEnabled) {
                ...
                var modalInstance = $uibModal.open({
                    ...
                    controller: ['$scope', '$uibModalInstance', 'msg', 'msgParams', '$translate', function ($scope, $uibModalInstance, msg, msgParams, $translate) {
                        $translate(msg).then(function (translatedMsg) {
                            if (!msgParams) {
                                $scope.msg = translatedMsg;
                            } else {
                                $translate(msgParams).then(function (result) {
                                    var index = 0;
                                    for (var key in result) {
                                        translatedMsg = translatedMsg.replace("{" + index + "}", result[key]);
                                        index += 1;
                                    }
                                    $scope.msg = translatedMsg;
                                })
                            }
                        }, function (notTranslatedMsg) {
                            $scope.msg = notTranslatedMsg;
                        });
            ...
            var DialogModal = function (templateUrl, paramObj, controller, size, animationsEnabled, controllerAs) {
                animationsEnabled = animationsEnabled || true;
                var modalInstance = $uibModal.open({
                    animation: animationsEnabled,
                    backdrop: 'static',
