...
namespace ESC5.WebCommon
{
    ...
    public class WebApiGuard<TUser> : IAuthorizationFilter, IFormTokenGenerator, ILoginTokenGenerator, IOrderedFilter
    {
        ...
        protected IGenericIdentity<TUser> Identity;
        protected IBaseSessionHelper SessionHelper;
        protected IDbDataEncrypter Encrypter;
        protected IApplicationStorage AppStorage;
        ...
        private  ActionResult? VerifyDuplicate(int delayRequest, AuthorizationFilterContext filterContext)
        {
            ...
            {
                key+=JsonConvert.SerializeObject(request.Form);
            }
        ...
        #region "辅助方法"
        /// <summary>
        /// 目前客户端框架未内置ClientType，必要时再加，然后可以使用此方法。
        /// </summary>
        ...
        protected virtual ActionResult? CheckLoginToken(HttpRequest request, HttpResponse response, string actionCode)
        {
            ...
            var ignore = AppStorage.GetAppSetting("PB_IgnoreLoginToken");
            if (Env.IsDevelopment() && ignore!=null && ignore.ToUpper() == "DEV") return null;
