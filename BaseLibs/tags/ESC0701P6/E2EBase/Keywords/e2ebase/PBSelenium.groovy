...
class PBSelenium extends WebDriverBackedSelenium{
	...
	@Keyword
	def testApi(WebApiTestCase webapi) {
		testApi(webapi,false);
	}
	@Keyword
	def testApi(WebApiTestCase webapi,boolean byPb) {
		ClientScriptUtil.doCallAction(webapi,byPb);
		verifyApi(webapi);
	}
	@Keyword
	def verifyApi(WebApiTestCase webapi) {
		if(webapi.result!=null) {
			RcResult actual= ClientScriptUtil.getRcResult(webapi.aca);
