    ...
    public class ClientDataTypeModelValidatorProvider : IClientModelValidatorProvider
    {
        ...
        internal sealed class NumericModelValidator : IClientValidatable, IClientModelValidator
        {
            public void AddValidation(ClientModelValidationContext context)
            {
                ...
                if (context.Attributes.ContainsKey("type") && context.Attributes["type"].Equals("hidden", StringComparison.OrdinalIgnoreCase))
                {
                    return;
                }
                ...
                foreach (var rule in rules)
                {
                    switch (rule.ValidationType)
                    {
                        case "pb-Grequired":
                            ...
                            break;
                        case "number":
                            if (!context.Attributes.ContainsKey("type") || context.Attributes["type"].Equals("text",StringComparison.OrdinalIgnoreCase))
                            {
