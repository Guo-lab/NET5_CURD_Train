...
namespace ProjectBase.Web.Mvc
{
    ...
    public abstract class BaseMvcApplication
    {
        ...
        public virtual Task Application_Error(HttpContext context)
        {
            ...
            string msg;
            if (context.Items.ContainsKey(BaseControllerExceptionFilter.KEY_EXCEPTION_LOGGED))
            {
                msg = context.Items[BaseControllerExceptionFilter.KEY_EXCEPTION_LOGGED].ToString();
            }
            else 
            { 
                var allMsg = "";
            ...
            if (!Env.IsDevelopment())
            {
                var html = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=utf-8' /></head><body><h2>系统运行错误，请联系管理员</h2></body></htm>";
        ...
        private void RunDevTool()
        {
            ...
            if (ProjectHierarchy.NamespaceMapToTablePrefix==null || ProjectHierarchy.NonprefixMvcModuleNames == null)
            {
                Console.WriteLine("开发工具未运行：未配置ProjectHierarchy.NamespaceMapToTablePrefix和ProjectHierarchy.NonprefixMvcModuleNames");
                return;
            }
            Console.WriteLine("开发工具运行：DevTool *******************************************");
            try
            {
                ...
                Console.WriteLine("开发工具运行：DevTool Succeeded*********************************");
            }
            catch (Exception e)
            {
                Console.WriteLine("开发工具运行错：DevTool Exception " + e.StackTrace);
            }
        }
