...
namespace ProjectBase.Web.Mvc.Angular
{
    public static class HtmlHelperExtension
    {
        ...
        //根据VM标记设置锁定
        public static void SetEnableByVMAnnotation<TModel, TProperty>(this IHtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, IDictionary<string, object> htmlAttributes)
        {
            var expresionProvider = HtmlHelperUtil.GetExpressionProvider(htmlHelper);
            ...
            if (enableAttr != null)
            {
                ...
                {
                    if (!enableAttr.ReadOnly.Value)
                    {
                        htmlAttributes["k-ng-disable"] = true;
                    }
                }
                else
                {
                    if (string.IsNullOrEmpty(enableAttr.ListName) == false)
                    {
                        ...
                        {
                            htmlAttributes["ng-disabled"] = htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.ListName + "['" + enableAttr.FieldName + "']";
                        }
                        else
                        {
                            htmlAttributes["ng-disabled"] = htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.ListName + "['" + meta.PropertyName + "']";
                        }
                    }
                    else
                    {
                        htmlAttributes["ng-disabled"] = htmlHelper.ViewContext.ViewData[Attr_VMPrefix] + "." + enableAttr.FieldName;
                    }
                }
            }
        }
