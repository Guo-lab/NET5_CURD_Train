...
namespace ProjectBase.Data
{
    ...
    public class UtilQuery : NHibernateQuery, IUtilQuery
    {
        ...
        public IList<TDto> StatelessGetDtoList<TEntity,TDto>(Expression<Func<TEntity, bool>> where, Expression<Func<TEntity, TDto>> selector)
        {
            IStatelessSession session = GetStatelessSession();
            try
            {
                return session.Query<TEntity>().Where(where).Select(selector).ToList();
            }
            catch (Exception)
            {
                throw;
        ...
        public IList<TEntity> StatelessGetList<TEntity>(Expression<Func<TEntity, bool>> where)
        {
            IStatelessSession session = GetStatelessSession();
            try
            {
        ...
        public TScalar StatelessGetScalar<TEntity,TScalar>(Expression<Func<TEntity, bool>> where, Expression<Func<TEntity, TScalar>> selector,bool unique=false)
        {
            IStatelessSession session = GetStatelessSession();
            var q = session.Query<TEntity>().Where(where).Select(selector);
            try
            {
                return unique?q.SingleOrDefault():q.FirstOrDefault();
            }
            catch (Exception)
            {
                throw;
        ...
        public int StatelessDelete<TEntity, TId>(Expression<Func<TEntity,bool>> where) where TEntity: IEntityWithTypedId<TId>
        {
            Check.Require(where != null);
            return StatelessDeleteInternal(default(TId), where);
        }

        public int StatelessDelete<TEntity, TId>(TId entityId, Expression<Func<TEntity, bool>> extraWhere) where TEntity : IEntityWithTypedId<TId>
        {
            Check.Require(entityId != null && extraWhere != null);
            return StatelessDeleteInternal(entityId, extraWhere);
        }

        private int StatelessDeleteInternal<TEntity, TId>(TId entityId, Expression<Func<TEntity, bool>> extraWhere) where TEntity : IEntityWithTypedId<TId>
        {
            Expression<Func<TEntity, bool>> filter = extraWhere;
            if (entityId!=null && !entityId.Equals(default(TId)))
            {
                var parameterExpr = Expression.Parameter(typeof(TEntity), "o");
                var idFilter = Expression.Lambda<Func<TEntity, bool>>(
            ...
            try
            {
                ...
                int rtn = session.Query<TEntity>().Where(filter).Delete();
                tx.Commit();
                return rtn;
            }
            catch (Exception)
