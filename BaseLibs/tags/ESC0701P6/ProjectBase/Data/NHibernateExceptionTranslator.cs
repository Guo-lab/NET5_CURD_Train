...
namespace ProjectBase.Data
{
    public class NHibernateExceptionTranslator : NHibernateQuery,IExceptionTranslator
    {
        ...
        public Exception Translate(Exception e)
        {
            ...
            if (!(e is ADOException || e is DbException)) return e;
            ...
            if (msg.IndexOf("UNIQUE KEY") >= 0 || msg.IndexOf("PRIMARY KEY") >= 0)
            {
                ...
                else
                {
                    ...
                    try 
                    {
                        var match = Regex.Match(msg, @"UQ_\w+");
                        if (match.Success)
                        {
                            constraintName = match.Value;
                        }
                        else 
                        { 
                            constraintName = Regex.Match(msg, @"PK_\w+").Value;
                        }
            ...
            else if (msg.IndexOf("REFERENCE") >= 0)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        constraintName = Regex.Match(msg, @"FK_\w+").Value;
                        msg = DBConstraintMap.ContainsKey(constraintName) ? DBConstraintMap[constraintName] : constraintName;
