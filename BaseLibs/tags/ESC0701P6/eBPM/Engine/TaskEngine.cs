...
namespace eBPM.Engine
{
                                                               ...
                                                               where ProcessorT : BaseCurrentProcessor, new()
    {
        ...
        public IJobCalculatorFactory jobCalculatorFactory { get; set; }
        ...
        protected virtual void CalculatePendingJob()
        {
            int orderId = WorkFlowObject.Id;
            string orderType = this.GetEntityName();
            IJobCalculator calculator = jobCalculatorFactory.GetJobCalculator(orderType);
            TransactionHelper.AddAsyncTask(() => TransactionHelper.DoInTrans(() =>
                 calculator.RefreshJob(orderId)
            ));
        }
