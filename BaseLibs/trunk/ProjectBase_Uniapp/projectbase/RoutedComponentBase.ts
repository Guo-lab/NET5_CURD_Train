 ...
 */
@Component
export class RoutedComponentBaseNoHook<Tvm = any, Tvminput = any> extends VmComponentBase<Tvm, Tvminput> {

    //////////routeData: any;
    //routeParam: NameValueObj;//移至VmComponentBase中
    ...
    OnNavBackRefresh(backSrc:string,backData:any) {}
    // // 返回前被调用
...
@Component
export class RoutedComponentBase<Tvm = any, Tvminput = any> extends RoutedComponentBaseNoHook<Tvm, Tvminput> {
   ...
   onBackPress(arg: any):void|boolean {
        return this.Override_onBackPress(arg);
    }
    onShow() {
        this.Override_onShow();
    }
    onReady() {
        this.SuperReady_ComponentBase();
    }
    protected Override_onBackPress(arg: any):void|boolean {
        this.Super_onBackPress(arg);
    }
    protected Override_onShow() {
        this.Super_onShow();
    }
    Super_onBackPress(arg: any,backData?:any) {
        try {
            const pages = getCurrentPages();
            const appInstance: BaseAppService = PBInjector.get(PBInjector.InjectToken.AppService);
            appInstance.navInfo.backSrc = pages[pages.length - 1].route||null;
    ...
    Super_onShow() {
        try {
            ...
            if (appInstance.navInfo.backSrc) {
                this.OnNavBackRefresh(appInstance.navInfo.backSrc, appInstance.navInfo.backData);
                appInstance.navInfo.backSrc = null;
                appInstance.navInfo.backData = null;
            }
        } catch (e) {
            pbErrorHandler.Throw(e);
        }
