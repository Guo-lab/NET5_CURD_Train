...
export class ProjectBaseService {
    ...
    private _pbPlugin: PBPlugInService|null=null;

    private vmMetas: { [vmTypeName: string]: VmMeta } = {};
    private vmMetasFb: { [vmTypeName: string]: PbAbstractControl|null } = {};
    ...
    AjaxSubmit(ajaxUrlorAjaxSubmitConfig: string | AjaxSubmitConfig | AjaxSubmitSimpleConfig,
        hostComponent?: VmComponentBase|null, form_?: VmForm|null, vmBind_?: VmComponentBase | boolean|null,
        funcExecuteResult?: ((result: any, argsForFunc?: any) => any | Promise<any>)|null,
        ajaxConfig_: AjaxSubmitConfig = {}): Promise<any> {

        let ajaxUrl: string | undefined;
        let formConfig: VmForm | boolean|undefined|null;
        ...
        let confirmKey: string | false | undefined|null;
        ...
        if (ajaxConfig && (ajaxConfig.confirm || ajaxConfig.confirmSingle || ajaxConfig.confirmMulti)) {
            ...
            return this.pbPlugin.ConfirmSubmit(confirmKey as string, confirmParams).then((confirmed) => {
    ...
    CallAction(actionUrl: string, params?: NameValueObj|null,
        funcExecuteResult?: ((result: any, argsForFunc?: any) => any | Promise<any>)|null,
        ...
        let ajaxBodyData = options.postJson ? params : util.Obj2Qs(params);
        let url = actionUrl;
        if (method.toLowerCase() == 'get') {
            ajaxBodyData = undefined;
            url = urlHelper.Action(url, params);
        } else {
        ...
        return this.doSubmit(url, method, ajaxBodyData, options.hostComponent, null, options, funcExecuteResult||null);
    }
    ...
    async Validate(formGroup: VmForm, valGroups?: string | string[]|null, partialValid?: string | string[]|null): Promise<string|null> {
        const ok = await this.autoValidator.validate(formGroup as VmForm, valGroups, partialValid);
        if (!ok) {
            let errs=(formGroup as PbAbstractControl).pbResolvedErrors;
            var msg= errs && errs.classValidate ? errs.classValidate.errMsg : ProjectBaseService.Event_AjaxSubmitValError;
    ...
    CheckVmInputType(vmTypeName: string): string|null {
        let rtn = null;
     ...
     * 创建表单验证规则对象
     */
    CreateFormGroup(vmInputTypeNameOrVmMeta: string | VmMeta, validateOn?: ValidateOnOpt|null, innerValidators?: {
        [name: string]: ValidatorFn|null;
    }): VmForm {
        try {
            let metafb: PbAbstractControl|null;
    ...
    */
    ...
    private CreateFormArrayInternal(containingFormGroup: VmForm, arrayPropNameOrVmInputTypeNameOfEle: string, arrayValue: any[] | number, classValidator ?: ClassValidator): FormArray {
        let arrayMeta: VmMeta|undefined;
        let vmInputTypeNameOfEle;
        let formArray: FormArray|null=null;

        if (containingFormGroup) {
            formArray = containingFormGroup.controls?containingFormGroup.controls[arrayPropNameOrVmInputTypeNameOfEle] as FormArray:null;
        ...
        let validators = containingFormGroup && arrayMeta ? this.getValidators(arrayMeta) : null;
        if (classValidator) {
        ...
        Check.Require(!!formArray.controls);
        controls.forEach((ctrl: PbAbstractControl|null, i: number) => {
            if(ctrl) {
                formArray!.controls![i] = ctrl;
            }
        });
    ...
    private restoreVmMeta(jsonObj: VmMetaMiniFied/*minified VmMeta*/) {
        ...
        meta.required = jsonObj.r||false;
        meta.translate = jsonObj.k;
        meta.formIgnore = jsonObj.f||false;
        meta.submitIgnore = jsonObj.s||false;
        if (jsonObj.v) {
            meta.rules = {};
    ...
    AddVmMeta(vmInputTypeName: string,meta:VmMeta) {
        return this.vmMetas[vmInputTypeName]=meta;
    }
    private addFormToken(formToken:string|null|undefined) {
        formToken=formToken||'';
    ...
    private ajaxSubmitMain(hostComponentInstance: VmComponentBase | any, ajaxUrl: string, ajaxConfig_?: AjaxSubmitConfig,
        form?: VmForm|null, vmBind?: VmComponentBase | boolean|null,
        funcExecuteResult?: ((result: any, argsForFunc?: any) => any | Promise<any>)|null): Promise<any> {

        // let processingAreaId;
        let method: string;
        let formValueContainingProp: string | boolean|null|undefined;
        let ajaxData: any;
        let ajaxBodyData;
        let url = ajaxUrl;
    ...
    private async doSubmit(url: string, method: string, ajaxBodyData: any, hostComponentInstance: VmComponentBase | any, form: VmForm|null|undefined, ajaxConfig: AjaxSubmitConfig,
        funcExecuteResult?:((result: any, argsForFunc?: any) => any | Promise<any>)|null): Promise<any> {

        let bindTarget: boolean | VmComponentBase|undefined;
        ...
        */
        ...
        const byRequestLoginToken = this.pbPlugin.AddLoginToken(ajaxConfig.loginToken,this.config,this.httpPostActionOptionsJson,this.httpPostActionOptions,this.httpGetActionOptions);
        ...
        let promise = request!.then((response: any[]) => {
            ...
            if (response[1]) {
                if (response[1].statusCode == 200) {
                    this.pbPlugin.SaveLoginToken(response,this.config);
                    return this.pbPlugin.ConvertResponseToRcResult(response[1].data);
                } else {
            ...
            try {
                if(responseData.isRcResult!=undefined){
            ...
            try {
                ...
                return responseData;
            } catch (err: any) {//此处是处理响应结果过程中的异常，不属于网络错误，因此用特殊方式表示
                throw {
                    name:'_wrappedError',
                    wrappedErr: err
                }
            ...
            } else if (err.name == '_wrappedError') {
                pbErrorHandler.Throw(err.wrappedErr);
            } else {
    ...
    private serializeAngularFormNv(formGroup: VmForm, formValueContainingProp: string): string {
        ...
        const meta = new VmMeta();
        meta.properties = {};
        Check.Require(!!(formGroup as VmForm).pbVmMeta)
    ...
    ProvideAnyData(data: AnyDataSubject | any, subjectName?: string|null, srcComponentType?: string|null, srcId?: string|null,
    ...
    private recursiveRemoveNullValueForFormGroup(vmFormorCtrl: PbAbstractControl, patchData: any) {
        Check.Require(!!vmFormorCtrl.controls);
    ...
    private getVmMetaFb(vmInputTypeName: string): PbAbstractControl|null {
        return this.vmMetasFb[vmInputTypeName];
    }
    private setVmMetaFb(vmInputTypeName: string, vmMetaFb: PbAbstractControl|null) {
        this.vmMetasFb[vmInputTypeName] = vmMetaFb;
    }
    private buildVmMetaFb(vmInputTypeName: string, validateOn?: ValidateOnOpt|null, innerValidators?: {
        [name: string]: ValidatorFn|null;
    }|null): PbAbstractControl|null{
     ...
     */
    // tslint:disable-next-line: cyclomatic-complexity
    private recursiveBuildVmMetaFb(meta: VmMeta, nodeName: string, parentMeta?: VmMeta|null, justForLeafEle:boolean|null = false, validateOn?: ValidateOnOpt|null, innerValidators?: {
        [name: string]: ValidatorFn|null;
    }|null): PbAbstractControl|null {
        ...
        let validators: any[]|null;//这里any指uni-forms的rule对象
    ...
    private getValidators(meta: VmMeta, justForLeafEle:boolean|null = false, valGroups?: string | string[]|null, innerValidators?: {
        [name: string]: ValidatorFn|null;
    }|null) {
    ...
    private shouldVal(valRule: ValidationRule, valGroups?: string | string[]|null) {
    ...
    public buildRuleConfig(form: VmForm, valGroups?: string | string[]): FormGroupRuleConfig {
        Check.Require(!!form.pbVmMeta,'未找到form.pbVmMeta');
        return this.recursiveBuildRuleConfig(form.pbVmMeta!, null, null, valGroups, form.innerValidators) as FormGroupRuleConfig;
    }
    private recursiveBuildRuleConfig(meta: VmMeta, nodeName: string|null, parentMeta?: VmMeta|null, valGroups?: string | string[], innerValidators?: {
        [name: string]: ValidatorFn|null;
    }|null): ControlRuleConfig | FormGroupRuleConfig {
