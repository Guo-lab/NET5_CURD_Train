...
export class UrlHelper {
    ...
    NavRoot(anyUrl: string, params?: NameValueObj, options?: UniApp.ReLaunchOptions) {
        ...
        Check.Require(!!url,'url计算错：'+anyUrl);
    ...
    NavTo(anyUrl: string, params?: NameValueObj, options?: UniApp.RedirectToOptions) {
        ...
        Check.Require(!!urlonly,'构造链接错：'+anyUrl);
        
        urlonly = urlonly!.substring(1);
        const pages = getCurrentPages();
        let i = pages.length - 1;
        let page = pages[i];
        Check.Require(!!page.route,'路由数据错');
    ...
    Link(anyUrl: string, params?: NameValueObj, pbFunc?: true | string) {
        ...
        let _anyUrl: string|null = anyUrl;////////// this.checkPbFunc(anyUrl, _params, pbFunc);
  ...
  Action(anyUrl: string, params?: NameValueObj|null) {
        params = params || {};
    ...
    */
    ...
    private generateUrl(anyUrl: string, qsParams?: NameValueObj|null, forState?: boolean) {
        let _anyUrl = anyUrl;

        const delimiter = '/';
        if (!_anyUrl.startsWith(delimiter) && !_anyUrl.startsWith('http://') && !_anyUrl.startsWith('https://')) {
            const stack = getCurrentPages();
            Check.Require(!!stack[stack.length - 1].route,'未找到路由数据');
    ...
    private mergeUrlParams(url: string, qsParams?: NameValueObj|null) {
        const urlfull = url.split('?');
