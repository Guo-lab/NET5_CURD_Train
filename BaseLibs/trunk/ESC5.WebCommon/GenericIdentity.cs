...
namespace ESC5.WebCommon
{
    public class GenericIdentity<TUser> : IGenericIdentity<TUser> where TUser: BaseDomainObjectWithTypedId<int>, IUserBase
    {
        ...
        {
            Cache = cache;
            //每用户的每类客户端一个loginToken
            Cache.Set(LOGIN_TOKEN_STUB_CACHE, new ConcurrentDictionary<int, ConcurrentDictionary<string,LoginToken>>());
        }
        ...
        private ConcurrentDictionary<int, ConcurrentDictionary<string, LoginToken>> GetStore()
        {
            return Cache.Get<ConcurrentDictionary<int, ConcurrentDictionary<string, LoginToken>>>(LOGIN_TOKEN_STUB_CACHE);
        }
        private ConcurrentDictionary<string, LoginToken>? GetTokenMap(int loginId)
        {
            if (GetStore().TryGetValue(loginId, out ConcurrentDictionary<string, LoginToken>? tokens))
            {
                return tokens;
            }
            return null;
        ...
        public void SaveLoginTokenStub(LoginToken token)
        {
            GetStore().AddOrUpdate(token.LoginId,
                (loginId) =>
                {
                    var tokenMap = new ConcurrentDictionary<string, LoginToken>();
                    tokenMap.TryAdd(GetClientType(), token);
                    return tokenMap;
                },
        ...
        public void RemoveLoginTokenStub(int loginId)
        {
            ...
            tokenMap.TryRemove(GetClientType(),out _);
            if (tokenMap.Count == 0)
            {
                GetStore().TryRemove(loginId,out _);
            }
        }
