...
namespace ProjectBase.Application
{
    public class CastleWindorComponentRegistrar
    {
        ...
        private static void AddOtherTo(IWindsorContainer container)
        {
            ...
            var dependencyResolver = new DependencyResolver(container.Kernel);
            container.Register(
                 Component.For(typeof(DependencyResolver))
                    .Instance(dependencyResolver)
            );
            container.Kernel.Resolver.AddSubResolver(dependencyResolver);
        }
    }
}
