...
namespace ProjectBase.Web.Mvc.Tool
{
    ...
    public class ClientVmMetadataProvider
    {
        ...
        private static List<Type> intTypes = new List<Type>() {
        ...
        //private static void ConvertMetas(SortedDictionary<string, ClientVmMetadata> metas)
        //{
        //    //System.Console.WriteLine(metas.Values.SelectMany(o => o.ValidationRules.SelectMany(y => y.Key).Distinct()).Distinct().ToList());
        //    foreach (var meta in metas)
        //    {
        //        System.Console.WriteLine(meta.Value.ValidationRules?.SelectMany(o => o.Key).Distinct());
        //    }
        //}
                ...
                IEnumerable<ModelMetadata> metaProperties, string propName, Type eleType)
        {
            ...
            if (eleType == null)
            {
                ...
                if (meta != null && Util.IsNullableType(modelClass))
                {
                ...
                if (properties.Count() > 0)
                {
                    ...
                    foreach (var propmeta in properties)
                    {
                        ...
                        else if (typeof(ICollection).IsAssignableFrom(propmeta.ModelType))
                        {
                            ...
                            {
                                if (propmeta.PropertyName != "selectedValues" && propmeta.ModelType != typeof(Array) && propmeta.ModelType != typeof(ArrayList))
                                {
                                    System.Diagnostics.Trace.WriteLine("class=" + metaForClass.FullName + ",  prop= " + propmeta.PropertyName);
                                    throw;
                                }
        ...
        private static List<BaseModelClientValidationRule> getClientValidationRules(ModelMetadata meta)
        {
            ...
            //转换number和string
            var numberRule = rules.Where(o => o.ValidationType.Equals("number", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if(numberRule!=null && intTypes.Contains(meta.ModelType))
            {
                rules.Add(new BaseModelClientValidationRule("int", false));
            }
            rules.Remove(numberRule);
            var trimAttr = meta.ContainerMetadata?.GetMetadataForProperty(meta.ContainerType,meta.PropertyName).ModelType.GetCustomAttributes(false).OfType<TrimAttribute>().FirstOrDefault();
            if(meta.ModelType ==typeof(string) && (trimAttr==null || trimAttr.Value)) {
                rules.Add(new BaseModelClientValidationRule("trim", false));
            }
            ...
            //转换required
            var grequired = rules.Where(o => o.ValidationType.Equals("pb-Grequired", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (grequired!=null || meta.IsRequired && rules.All((rule) => rule.ValidationType != "required"))
            {
                rules.Add(new BaseModelClientValidationRule("required", false));
            }
            if (grequired != null)
            {
                rules.Remove(grequired);
            }

            //转换range
            var rangeRule = rules.Where(o => o.ValidationType.Equals("pb-Range", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (rangeRule != null)
            {
                if (rangeRule.ValidationParameters["max"] != null)
                {
                    var rule = new BaseModelClientValidationRule("max", rangeRule.ErrorMessage, rangeRule.Groups, true);
                    rule.ValidationParameters["max"] = rangeRule.ValidationParameters["max"];
                    rules.Add(rule);
                }
                if (rangeRule.ValidationParameters["min"] != null)
                {
                    var rule = new BaseModelClientValidationRule("min", rangeRule.ErrorMessage, rangeRule.Groups, true);
                    rule.ValidationParameters["min"] = rangeRule.ValidationParameters["min"];
                    rules.Add(rule);
                }
                rules.Remove(rangeRule);
            }
        ...
        private static string GetModuleName(Type metaForClass)
        {
            ...
            {
                var mdname= ProjectHierarchy.GetModuleName.Invoke(metaForClass);
                if (mdname != null) return mdname;
            }

            var areaAttr=metaForClass.GetCustomAttributes(false).OfType<PBAreaAttribute>().SingleOrDefault();
