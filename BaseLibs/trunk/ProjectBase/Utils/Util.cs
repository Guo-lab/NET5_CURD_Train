...
namespace ProjectBase.Utils
{
    public class Util : IUtil
    {
        ...
        public string NgControllerJs()
        {
            ...
            var fileNames = Directory.EnumerateFiles(contentRoot, "*Ctrl.js", SearchOption.AllDirectories).Where(x => !x.Contains("\\bin") && !x.Contains("\\obj") && !x.Contains("\\ProjectBase"));
            var scripts = fileNames.OrderBy(x=>Path.GetDirectoryName(x)).ThenBy(x => Path.GetFileNameWithoutExtension(x)).Aggregate("", (aggregate, fileName) =>
                                  {
