...
class PBSelenium extends WebDriverBackedSelenium{
	...
	def ClientScriptUtil ClientScriptUtil;

	PBSelenium(){
		super(DriverFactory.getWebDriver(), "http://localhost")
		EleUtil=new ElementUtil(this);
		ClientScriptUtil=new ClientScriptUtil(this);
	}
	...
	//--------------包装过时接口并增强------------
	def void formInput(String locatorPrefix,Map<String,Object> value) {
		//根据数据key对应绑定的控件，给控件赋值
	}
	...
	private def WebElement findElement(String locator) {
		...
		if(pos==-1) {
			try {
				element = this.wrappedDriver.findElement(By.id(locator));
			}catch(Exception e) {
				element = this.wrappedDriver.findElement(By.name(locator));
			}
			return element;
		}
	 ...
	 */
	private def smartLocate(String key) {
	...
	private def WebElement wait2type(String locator,String value) {
		WebElement ele=wait2click(locator);
	...
	def doInController(String script) {
		ClientScriptUtil.doInController(script);
	}
	@Keyword
	def doInController(String script,Object[] params) {
		ClientScriptUtil.doInController(script,params);
	}
	@Keyword
	def setInController(String scriptLeft,Map<String,Object> objRight) {
		String scriptRight=JsonOutput.toJson(objRight);
		ClientScriptUtil.doInControllerApply(scriptLeft+"="+scriptRight);
	}
	@Keyword
	def setInController(String scriptLeft,Object objRight) {
		String scriptRight="";
		if(objRight==null){
			scriptRight="null";
		} else {
			scriptRight=objRight instanceof String?"'"+objRight+"'":objRight.toString();
		}
		ClientScriptUtil.doInControllerApply(scriptLeft+"="+scriptRight);
	}
	@Keyword
	def Map<String,Object> getInController(String scriptExpr) {
		return ClientScriptUtil.getInController(scriptExpr);
	}
	@Keyword
	def enter(WebApiTestCase webapi) {
		if(webapi.url==null) {
			webapi.url=webapi.aca;
		}
		if(webapi.url.startsWith("/")) {
			webapi.url=GlobalVariable.ClientBaseUrl+webapi.url;
		}
		this.open(webapi.url);
		verifyApi(webapi);
	}
	@Keyword
	def RcResult testApi(WebApiTestCase webapi) {
		return testApi(webapi,null,false);
	}
	@Keyword
	def RcResult testApi(WebApiTestCase webapi,boolean byPb) {
		return testApi(webapi,null,byPb);
	}
	@Keyword
	def RcResult testApi(WebApiTestCase webapi,Integer timeoutMillisec,boolean byPb) {
		ClientScriptUtil.doCallAction(webapi,byPb);
		return verifyApi(webapi,timeoutMillisec);
	}
	@Keyword
	def RcResult verifyApi(WebApiTestCase webapi) {
		return verifyApi(webapi,null);
	}
	@Keyword
	def RcResult verifyApi(WebApiTestCase webapi,Integer timeoutMillisec) {
		RcResult actual=null;
		if(webapi.result!=null) {
			actual= ClientScriptUtil.getRcResult(webapi.aca,timeoutMillisec);
			ClientScriptUtil.verifyRcResult(actual,webapi.result);
		}
		return actual;
	}
}