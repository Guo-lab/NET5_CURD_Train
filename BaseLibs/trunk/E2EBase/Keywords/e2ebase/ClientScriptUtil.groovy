...
public class ClientScriptUtil {
	...
	def doCallAction(WebApiTestCase webapi,String ajaxOpnJson,boolean byPb) {
		...
		String script="PB_Global_WebApiTestingInterceptor.TestingCallAction('"+webapi.url+"',"+bodyJson+","+ajaxOpnJson+","+sByPb+",'"+webapi.aca+"');";
		WebUI.executeJavaScript(script,null);
		BaseRunner.logStep("doCallAction",script);
	}
	def RcResult getRcResult(String aca) {
		return getRcResult(aca,null);
	}
	def RcResult getRcResult(String aca,Integer timeoutMillisec) {
		BaseRunner.setStepLogDT();
		boolean exists=wait({
		...
		BaseRunner.logStep("getRcResult",aca);
		return result;
	}
