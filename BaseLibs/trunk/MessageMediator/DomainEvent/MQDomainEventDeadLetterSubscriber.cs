    ...
    public class MQDomainEventDeadLetterSubscriber
    {
        ...
        public void Init(string publisherName) 
        {
            ...
            try
            {
                ...
                var connName = DomainEventOutProcSubscriber?.ConnectionName?? MQDomainEventSubscriber.KEY_SUBSCRIBER_CONNECTION_PREFIX + publisherName;
                connection = RabbitMQConnectionManager.Connect(connName);
                Subscribe();
            }
            catch (Exception e)
            {
                AlarmReporter.Error("MQDomainEventDeadLetterSubscriber启动异常：", e);
            }
        }
